...
import TransitionsSnackbar from '../../Error Statements/Error';
...
export const DraggableDialog = ({ roundManager }) => {
  ...
  const handleConversion = (event) => {
    if (Number(rRes) + Number(bRes) + Number(yRes) != 4) {
      setMessage("Your values don't equal to four")
      setError(true)
    } else if (ExChange != 'y' && ExChange != 'r' && ExChange != 'b') {
      setMessage("You forgot to set the resource you wish to exchange for")
      setError(true)
    } else if (rRes > roundManager.roundResources[0] || yRes > roundManager.roundResources[1] || bRes > roundManager.roundResources[2]) {
      setMessage("You can't exchange nonexistant resources!")
      setError(true)
    } else {
